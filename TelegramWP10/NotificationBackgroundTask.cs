...
namespace TelegramWP10
{
    public sealed class NotificationBackgroundTask : IBackgroundTask
    {
        ...
        private IntPtr _client;
        private bool _authorized = false;
        private int _maxWaitMs = 20000; // 20 секунд максимум
        ...
        private async Task RunAsync() {
            ...
            string filesPath = settings.Values["bg_files_path"] as string;
            long apiId = 26688287;
            string apiHash = "5f4afe72bc71dc6ec40f7dcb0c9a822b";
            ...
            SendJson("{\"@type\":\"setTdlibParameters\"" +
                ",\"use_test_dc\":false" +
                ",\"database_directory\":\"" + EscapeJson(dbPath) + "\"" +
                ",\"files_directory\":\"" + EscapeJson(filesPath ?? dbPath + "_files") + "\"" +
                ",\"use_file_database\":true" +
            ...
            while (elapsed < _maxWaitMs) {
                IntPtr ptr = TdJson.Receive(_client, 1.0);
                if (ptr != IntPtr.Zero) {
                    string json = TdJson.IntPtrToStringUtf8(ptr);
                    if (!string.IsNullOrEmpty(json)) {
                        ProcessUpdate(json);
                ...
                await Task.Delay(pollInterval);
                elapsed += pollInterval;
                if (!_authorized) elapsed += pollInterval; // быстрее отваливаемся если не авторизованы
            }
        ...
        private void ProcessUpdate(string json) {
            try {
                // Простой парсинг без Newtonsoft для минимизации зависимостей
                if (json.Contains("\"updateAuthorizationState\"")) {
                    if (json.Contains("authorizationStateReady"))
                        _authorized = true;
                    else if (json.Contains("authorizationStateClosed"))
                        _maxWaitMs = 0; // выходим сразу
                }

                if (!_authorized) return;

                if (json.Contains("\"updateNewMessage\"")) {
                    // Проверяем что не наше сообщение
                    if (json.Contains("\"is_outgoing\":true")) return;
        ...
        private string ExtractSenderName(string json) {
            try {
                // Ищем first_name в JSON
                int idx = json.IndexOf("\"first_name\":\"");
                if (idx >= 0) {
                    idx += 14;
                    int end = json.IndexOf("\"", idx);
                    if (end > idx) return json.Substring(idx, end - idx);
                }
                // Для группы — title
                idx = json.IndexOf("\"title\":\"");
                if (idx >= 0) {
        ...
        private string ExtractMessageText(string json) {
            try {
                ...
                if (idx >= 0) {
                    ...
                    int end = json.IndexOf("\"", idx);
                    if (end > idx) {
                        string t = json.Substring(idx, end - idx);
        ...
        private void SendJson(string json) {
            try { TdJson.SendUtf8(_client, json); } catch { }
        }
